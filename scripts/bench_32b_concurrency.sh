...
#
# 3 CUDA graph settings x 8 concurrency levels = 24 data points.
#
...
print()
print('=' * 100)
print('  PREFILL THROUGHPUT (tokens/s) — Qwen2.5-32B, 1 prefill GPU (H200)')
print('=' * 100)
...
for s in settings:
    ...
    for cc in ccs:
        ...
        if os.path.exists(f):
            with open(f) as fh:
                d = json.load(fh)
            row += f'  {d[\"prefill_throughput_tok_s\"]:>8.0f}'
        else:
            row += f'  {\"N/A\":>8s}'
    print(row)

print()
print('  MEDIAN TTFT (ms)')
print('-' * 100)
hdr = f\"{'Setting':<22s}\"
for cc in ccs:
    hdr += f'  cc={cc:<5d}'
