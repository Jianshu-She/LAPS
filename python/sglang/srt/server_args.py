...
class ServerArgs:
    ...
    laps_wait_window_ms: float = 5.0
    laps_wait_max_batch: int = 8
    torchao_config: str = ""
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--laps-wait-window-ms",
            type=float,
            default=ServerArgs.laps_wait_window_ms,
            help="Waiting window in milliseconds for short requests to accumulate before dispatch (default: 5.0).",
        )
        parser.add_argument(
            "--laps-wait-max-batch",
            type=int,
            default=ServerArgs.laps_wait_max_batch,
            help="Maximum batch size target for short requests during waiting window (default: 8).",
        )
        parser.add_argument(
            "--torchao-config",
            type=str,
