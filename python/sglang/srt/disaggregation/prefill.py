...
class SchedulerDisaggregationPrefillMixin:
    ...
    def _init_laps_state(self: Scheduler) -> None:
        ...
        self._laps_consecutive_short_count = 0
        self._laps_max_consecutive_short = (
            self.server_args.laps_max_consecutive_short
        )
        logger.info(
            f"LAPS scheduler enabled with length threshold={self._laps_threshold}, "
            f"max_consecutive_short={self._laps_max_consecutive_short}"
        )
    ...
    def _get_next_laps_batch_to_run(
        ...
        """Decides which queue to service with anti-starvation for long queue."""
        self.running_batch.batch_is_full = False
        self.process_prefill_chunk()

        batch = None
        force_long = (
            self._laps_consecutive_short_count >= self._laps_max_consecutive_short
            and len(self._laps_long_queue) > 0
        )
        ...
        if batch is None and len(self._laps_short_queue) > 0:
            batch = self._build_batch_from_queue(self._laps_short_queue)
            if batch is not None:
                self._laps_consecutive_short_count += 1
