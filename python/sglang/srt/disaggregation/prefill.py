...
class SchedulerDisaggregationPrefillMixin:
    ...
    def _init_laps_state(self: Scheduler) -> None:
        ...
        self._laps_wait_window_s = self.server_args.laps_wait_window_ms / 1000.0
        self._laps_wait_max_batch = self.server_args.laps_wait_max_batch
        self._laps_short_wait_start: Optional[float] = None
        logger.info(
            f"LAPS scheduler enabled with length threshold={self._laps_threshold}, "
            f"wait_window={self._laps_wait_window_s * 1000:.1f}ms, "
            f"wait_max_batch={self._laps_wait_max_batch}"
        )
    ...
    def _laps_classify_requests(self: Scheduler, reqs: List[Req]) -> None:
        ...
        # Start the waiting window timer when first short request enters an empty queue
        if self._laps_short_wait_start is None and len(self._laps_short_queue) > 0:
            self._laps_short_wait_start = time.perf_counter()
    ...
    def _laps_should_dispatch_short(self: Scheduler) -> bool:
        """Check if short queue should be dispatched based on waiting window."""
        # Trigger 1: batch size target reached
        if len(self._laps_short_queue) >= self._laps_wait_max_batch:
            return True
        # Trigger 2: window expired
        if self._laps_short_wait_start is not None:
            if time.perf_counter() - self._laps_short_wait_start >= self._laps_wait_window_s:
                return True
        # Not ready yet — process long queue instead
        return False

    def _laps_is_waiting(self: Scheduler) -> bool:
        """Return True if we are in a waiting window for short requests."""
        return (
            len(self._laps_short_queue) > 0
            and not self._laps_should_dispatch_short()
        )

    def _get_next_laps_batch_to_run(
        self: Scheduler,
    ) -> Optional[ScheduleBatch]:
        """Decides which queue to service: short (with waiting window) first, then long."""
        self.running_batch.batch_is_full = False
        self.process_prefill_chunk()

        batch = None
        # Short queue has priority, but respect waiting window
        if len(self._laps_short_queue) > 0:
            if self._laps_should_dispatch_short():
                batch = self._build_batch_from_queue(self._laps_short_queue)
                if batch is not None:
                    # Reset timer: restart if there are still short requests remaining
                    self._laps_short_wait_start = (
                        time.perf_counter() if len(self._laps_short_queue) > 0 else None
                    )

        # Fall back to long queue (also serves long requests during short waiting window)
    ...
    def event_loop_normal_disagg_prefill(self: Scheduler) -> None:
        ...
        while True:
            ...
            if batch:
                ...
                # Don't trigger idle check during LAPS waiting window
                if not (use_laps and self._laps_is_waiting()):
