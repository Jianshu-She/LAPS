#!/bin/bash
# Experiment 18: Eager Fallback vs Piecewise CG Fallback
#
# When batch_prefill CG misses, should we fall back to:
#   B: piecewise CG (N individual graph replays with padding)  [CURRENT]
#   E: vanilla eager (native flash attn, no padding)           [NEW]
#
# Both settings use batch_prefill CG + queue-depth gate.
# The ONLY difference is the fallback path when batch_prefill CG misses.
#
# Model: Qwen/Qwen2.5-7B-Instruct, TP=1
# Dataset: lmsys-chat-1m (10000 prompts)
# Concurrency: 4, 8, 32, 64
#
# Usage: bash bench_exp18.sh <vanilla|B|E|all> [GPU_ID]
#
set -euo pipefail

PHASE="${1:?Usage: bash bench_exp18.sh <vanilla|B|E|all> [GPU_ID]}"
...
BASE_DIR="${SCRIPT_DIR}/results_exp18_${TODAY}"
mkdir -p "$BASE_DIR"
...
echo "Experiment 18: Eager Fallback vs Piecewise CG Fallback"
echo "  Phase:       ${PHASE}"
echo "  GPU:         ${GPU}"
echo "  Port:        ${PORT}"
...
# ═══════════════════════════════════════════════════════════════════════
#  Approach B: batch_prefill CG + piecewise CG fallback (CURRENT)
#  When batch_prefill CG misses → piecewise CG (N individual replays)
# ═══════════════════════════════════════════════════════════════════════

run_approach_b() {
    echo ""
    echo "################################################################"
    echo "  B: piecewise_fallback (batch_prefill CG miss → piecewise CG)"
    echo "################################################################"

    SDIR="${BASE_DIR}/piecewise_fallback"
    mkdir -p "$SDIR"
    # Use piecewise-extend-max-bs 9999 to NOT gate piecewise CG by batch size
    # This is the CURRENT behavior: batch_prefill miss → piecewise CG
    launch_server "$SDIR" "piecewise_fallback" \
        $CG_COMMON $CG_GRIDS \
        --piecewise-extend-max-bs 9999 \
        --piecewise-max-waiting-reqs 9999
    warmup "$SDIR"
    run_sweep "$SDIR" "piecewise_fallback"
}

# ═══════════════════════════════════════════════════════════════════════
#  Approach E: batch_prefill CG + eager fallback (NEW)
#  When batch_prefill CG misses → vanilla eager (native flash attn)
#  Achieved by: batch_prefill_enabled=True skips piecewise CG check
# ═══════════════════════════════════════════════════════════════════════

run_approach_e() {
    echo ""
    echo "################################################################"
    echo "  E: eager_fallback (batch_prefill CG miss → vanilla eager)"
    echo "################################################################"

    SDIR="${BASE_DIR}/eager_fallback"
    mkdir -p "$SDIR"
    # With the code change, when batch_prefill_enabled=True, piecewise CG
    # is automatically skipped, so misses go straight to eager.
    # No queue-depth gate needed since piecewise is bypassed anyway.
    launch_server "$SDIR" "eager_fallback" \
        $CG_COMMON $CG_GRIDS \
        --piecewise-extend-max-bs 9999 \
        --piecewise-max-waiting-reqs 9999
    warmup "$SDIR"
    run_sweep "$SDIR" "eager_fallback"
}
...
case "$PHASE" in
    ...
    ALL)
        ...
        run_approach_e
        ;;
    *) echo "ERROR: Unknown phase '$PHASE'. Use vanilla, B, E, or all."; exit 1 ;;
esac
...
ccs = [4, 8, 32, 64]
settings = ['vanilla', 'piecewise_fallback', 'eager_fallback']
labels = {'vanilla': 'vanilla', 'piecewise_fallback': 'B: piecewise_fb', 'eager_fallback': 'E: eager_fb'}
...
for s in ['piecewise_fallback', 'eager_fallback']:
    row = f'{labels[s]:<25s}'
    for cc in ccs:
        d_v = load('vanilla', cc)
        d_s = load(s, cc)
...
print()
print('=' * 90)
print('  RPS Delta: E (eager_fb) vs B (piecewise_fb)')
print('=' * 90)
print(f'  {hdr2}')
print(f'  {\"-\" * len(hdr2)}')

row = f'{labels[\"eager_fallback\"]:<25s}'
for cc in ccs:
    d_b = load('piecewise_fallback', cc)
    d_e = load('eager_fallback', cc)
    row += f'  {fmt_pct(rps(d_e), rps(d_b))}'
print(f'  {row}')
...
echo ""
echo "================================================================"
echo "  EXPERIMENT 18 — PHASE ${PHASE} COMPLETE"
echo "  Results: ${BASE_DIR}/"
echo "================================================================"
