#!/bin/bash
# Benchmark prefill throughput on Qwen2.5-7B with 2 GPUs (TP=1) and concurrency sweep.
#
# 3 settings x 7 concurrency levels = 21 data points.
# Prefill server: GPU 0 (TP=1); Decode server: GPU 1 (TP=1).
#
# Usage: bash bench_7b_2gpu.sh
set -euo pipefail
...
launch_servers() {
    ...
    echo "  Prefill GPU: ${PREFILL_GPUS}, Decode GPU: ${DECODE_GPUS}"
