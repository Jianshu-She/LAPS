#!/bin/bash
# Benchmark prefill throughput on Qwen2.5-72B-Instruct with TP=2.
# Runs the full experiment NUM_RUNS times with different seeds.
#
# Each run's results are saved to:
#   results_72b_runs/YYYY-MM-DD_runN/
#
# After all runs, an aggregate summary (mean +/- std) is generated.
#
# Usage: bash bench_72b_multirun.sh
set -euo pipefail
...
BASE_RESULTS_DIR="${SCRIPT_DIR}/results_72b_runs"
...
SETTINGS_ARGS=("" "--enable-piecewise-cuda-graph" "--enable-piecewise-cuda-graph --enable-batch-prefill-cuda-graph" "$LAPS_ARGS" "--enable-piecewise-cuda-graph --enable-batch-prefill-cuda-graph $LAPS_ARGS")
