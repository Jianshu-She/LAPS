#!/bin/bash
# Experiment 17: Vanilla vs Approach B vs Approach B+D (length-sorted batching)
#
# Fair head-to-head comparison on same hardware, same session.
#   vanilla:       No CG optimizations (baseline)
#   B: queue_gate: batch_prefill CG + piecewise CG + queue-depth gate
#   B+D: queue_gate + length_sort: B + prefill-sort-window 64
#
# Model: Qwen/Qwen2.5-7B-Instruct, TP=1
# Dataset: lmsys-chat-1m (10000 prompts)
# Concurrency: 4, 8, 32, 64
#
# Usage: bash bench_exp17.sh <vanilla|B|BD|all> [GPU_ID]
#
set -euo pipefail

PHASE="${1:?Usage: bash bench_exp17.sh <vanilla|B|BD|all> [GPU_ID]}"
...
BASE_DIR="${SCRIPT_DIR}/results_exp17_${TODAY}"
mkdir -p "$BASE_DIR"
...
echo "Experiment 17: Vanilla vs Approach B vs Approach B+D (length_sort)"
echo "  Phase:       ${PHASE}"
echo "  GPU:         ${GPU}"
echo "  Port:        ${PORT}"
...
# ═══════════════════════════════════════════════════════════════════════
#  Approach B+D: Queue-depth gate + length-sorted batching
#  piecewise CG skipped when num_waiting_reqs > 8
#  batch-size gate disabled (9999)
#  prefill-sort-window 64 for length-aware batch construction
# ═══════════════════════════════════════════════════════════════════════

run_approach_bd() {
    echo ""
    echo "################################################################"
    echo "  Approach B+D: queue_gate + length_sort (--prefill-sort-window 64)"
    echo "################################################################"

    SDIR="${BASE_DIR}/queue_gate_length_sort"
    mkdir -p "$SDIR"
    launch_server "$SDIR" "queue_gate_length_sort" \
        $CG_COMMON $CG_GRIDS \
        --piecewise-extend-max-bs 9999 \
        --piecewise-max-waiting-reqs 8 \
        --prefill-sort-window 64
    warmup "$SDIR"
    run_sweep "$SDIR" "queue_gate_length_sort"
}
...
case "$PHASE" in
    ...
    ALL)
        ...
        run_approach_bd
        ;;
    *) echo "ERROR: Unknown phase '$PHASE'. Use vanilla, B, BD, or all."; exit 1 ;;
esac
...
ccs = [4, 8, 32, 64]
settings = ['vanilla', 'queue_gate', 'queue_gate_length_sort']
labels = {'vanilla': 'vanilla', 'queue_gate': 'B: queue_gate', 'queue_gate_length_sort': 'B+D: length_sort'}
...
for s in ['queue_gate', 'queue_gate_length_sort']:
    row = f'{labels[s]:<25s}'
    for cc in ccs:
        d_v = load('vanilla', cc)
        d_s = load(s, cc)
...
print()
print('=' * 90)
print('  RPS Delta B+D vs B (%)')
print('=' * 90)
print(f'  {hdr2}')
print(f'  {\"-\" * len(hdr2)}')

row = f'{labels[\"queue_gate_length_sort\"]:<25s}'
for cc in ccs:
    d_b = load('queue_gate', cc)
    d_bd = load('queue_gate_length_sort', cc)
    row += f'  {fmt_pct(rps(d_bd), rps(d_b))}'
print(f'  {row}')
...
echo ""
echo "================================================================"
echo "  EXPERIMENT 17 — PHASE ${PHASE} COMPLETE"
echo "  Results: ${BASE_DIR}/"
echo "================================================================"
