#!/bin/bash
# Full benchmark: Qwen2.5-32B, TP=1, all 5 settings × cc=1-128 × 5 runs
# LAPS uses N=1 (--laps-max-consecutive-short 1)
#
# Settings:
#   1. Vanilla SGLang
#   2. Prefill CUDA Graph (piecewise only)
#   3. Batch Prefill CUDA Graph
#   4. Prefill Disaggregation (no CUDA graph)
#   5. LAPS (anti-starvation N=1, with batch prefill CUDA graph)
#
# Usage: bash bench_32b_laps_n1_5runs.sh
set -euo pipefail
...
TODAY=$(date +%Y-%m-%d)
BASE_RESULTS_DIR="${SCRIPT_DIR}/results_32b_10k_${TODAY}"
...
===============================================================================
  Launch command: bash $(basename "$0")
  Script:        $(readlink -f "$0")
...
title = 'Qwen2.5-32B, TP=1 (1x H200) — ${run_label}'
print()
print('=' * 140)
...
# ───────────────────────── main: run NUM_RUNS iterations ─────────────────────────

SETTINGS_LABELS=("vanilla_sglang" "prefill_cuda_graph" "batch_prefill_cuda_graph" "prefill_disagg" "laps")
# For prefill server: --disable-cuda-graph skips decode graph capture (saves ~0.86 GB)
# Piecewise: use original sglang defaults (up to 8192 tokens)
# Batch prefill: bs=[1,2,4,8,16,32], seq=[1,2,4,8,16,32,64,128,256,512]
