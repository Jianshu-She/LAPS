#!/bin/bash
# Benchmark prefill throughput on Qwen2.5-32B with TP=4 and concurrency sweep.
# After each concurrency level, flush the KV cache on both prefill and decode servers.
#
# 5 settings x 6 concurrency levels = 30 data points.
# Prefill server: GPU 0,1,2,3 (TP=4); Decode server: GPU 4,5,6,7 (TP=4).
#
# Usage: bash bench_32b_flush_cache.sh
set -euo pipefail
...
RESULTS_DIR="${SCRIPT_DIR}/results_32b_tp4_flush"
...
title = 'Qwen2.5-32B, TP=4, 1 prefill instance (4x H200) [KV cache flushed between runs]'
print()
print('=' * 110)
