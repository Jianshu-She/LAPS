#!/bin/bash
# Benchmark: LAPS N=2 vs N=4 on Qwen2.5-32B, TP=1, 3 runs
# Runs on GPUs 4,5 with separate ports to avoid conflict with other benchmarks.
#
# Usage: bash bench_32b_laps_n2.sh
set -euo pipefail
...
BASE_RESULTS_DIR="${SCRIPT_DIR}/results_32b_laps_n2"
...
launch_servers() {
    ...
    CUDA_VISIBLE_DEVICES=$DECODE_GPU $PYTHON -m sglang.launch_server \
        ...
        --disaggregation-bootstrap-port 9400 \
    ...
    CUDA_VISIBLE_DEVICES=$PREFILL_GPU $PYTHON -m sglang.launch_server \
        ...
        --disaggregation-transfer-backend $BACKEND \
        --disaggregation-ib-device $IB_DEVICE \
        --disaggregation-bootstrap-port 9401 \
    ...
    $PYTHON -m sglang_router.launch_router \
        ...
        --prefill "http://${HOST}:${PREFILL_PORT}" 9401 \
...
SETTINGS_LABELS=("laps_n2" "laps_n4")
SETTINGS_ARGS=(
    "--enable-piecewise-cuda-graph --enable-batch-prefill-cuda-graph --enable-laps-scheduler --laps-length-threshold 256 --laps-max-consecutive-short 2"
    "--enable-piecewise-cuda-graph --enable-batch-prefill-cuda-graph --enable-laps-scheduler --laps-length-threshold 256 --laps-max-consecutive-short 4"
)
...
title = f'Qwen2.5-32B, TP=1 (1x H200) — LAPS N=2 vs N=4 — AGGREGATE ({num_runs} runs)'
lines = []
lines.append('')
lines.append('=' * 170)
